...
export default function (data) {
  ...
  // Mix of operations to stress test (same as performance test)
  const operation = Math.floor(Math.random() * 3);

  if (operation === 0) {
    // GET user list (requires Admin token) - NO Content-Type for GET
    const headers = {
    ...
    // First register a new user - nome não pode conter números
    ...
    // Register new User account - nome não pode conter números
    const uniqueId = `${__VU}${__ITER}${Date.now()}${Math.random().toString(36).substring(7)}`;
